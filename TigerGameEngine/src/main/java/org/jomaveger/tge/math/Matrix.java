...
public final class Matrix {
    ...
    public Matrix setRotationBy3Axis(Vector rotation)
	{
		float x = MathUtil.toRadians(rotation.x);
		float y = MathUtil.toRadians(rotation.y);
		float z = MathUtil.toRadians(rotation.z);
    ...
    public Matrix makeTransform(Vector translation, Vector rotation, Vector scaling) {
		...
		float cr = MathUtil.cos(rotation.x * 0.5f);
		float sr = MathUtil.sin(rotation.x * 0.5f);
		float cp = MathUtil.cos(rotation.y * 0.5f);
		float sp = MathUtil.sin(rotation.y * 0.5f);
		float cy = MathUtil.cos(rotation.z * 0.5f);
		float sy = MathUtil.sin(rotation.z * 0.5f);
		...
		float scalingX = scaling.x;
		float scalingY = scaling.y;
    ...
    public Matrix buildViewMatrix(Vector eye, Vector lookAt, Vector up) {
		this.identity();
    ...
    public Matrix buildPerspectiveMatrix(float width, float height, float near, float far, float angleInRadians) {
		this.zero();
